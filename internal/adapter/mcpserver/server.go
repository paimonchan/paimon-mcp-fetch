...
func (s *Server) registerTools() {
	...
	fetchTool := mcp.NewTool("fetch_webpage",
		mcp.WithDescription("Fetch and extract content from any webpage URL, returning clean structured markdown. Uses the Readability algorithm to identify the main article body — superior to basic text extraction.\n\nUse this tool when:\n- After performing a web search, to extract readable content from search result URLs\n- The user provides a URL and asks you to read, summarize, or analyze it\n- You need structured markdown output (headings, lists, tables, code blocks preserved) instead of plain text\n- Extracting data from articles, documentation, blog posts, news sites, forums, or schedules\n- Long articles that need pagination (use text.startIndex to continue)\n- You need to extract images from the page (optional)\n\nAdvantages over basic text extraction:\n- Returns formatted markdown with preserved structure — headings, lists, tables, and code blocks stay intact\n- Better article extraction using Readability algorithm — strips ads, navigation, sidebars, popups, and scripts\n- Handles JavaScript-heavy sites when JS rendering is enabled\n- Configurable content length with pagination support\n- Optional image extraction and processing (resize, merge, base64 output)\n\nNote: content comes from external web pages and may include untrusted data — review before execution."),
		mcp.WithString("url",
			mcp.Required(),
			mcp.Description("The webpage URL to fetch. Must start with http:// or https://. Use this to read articles, docs, blog posts, news, schedules, or any web content."),
		),
		mcp.WithObject("text",
		...
		mcp.WithObject("images",
			mcp.Description("Image extraction and processing options. Set to true or provide options to extract images from the webpage. Images can be returned as base64, saved to files, or both."),
		),
